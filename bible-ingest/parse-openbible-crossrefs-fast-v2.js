...
const PAGE_SIZE = 5000;     // fewer DB round-trips
...
function parseRef(ref) {
  ...
  // Most ranges keep the same book_ext; if not, we still store start.book_ext.
  return {
...
async function loadCanonOrderMap() {
  // IMPORTANT: your table uses book_code (not code)
  const { data, error } = await supabase
  ...
  const map = new Map();
  for (const r of data) map.set(r.book_code, r.canon_order);
  return map;
}
...
async function main() {
  ...
  while (true) {
    ...
    console.log(
      `Fetched ${totalFetched} raw… upserted ${totalUpserted} parsed… skipped ${totalSkipped}…`
    );

    offset += PAGE_SIZE;
  }

  console.log('DONE parsing crossrefs (fast v2).');
  console.log(`Total fetched: ${totalFetched}`);
  console.log(`Total parsed/upserted: ${totalUpserted}`);
