...
// ===== CONFIG =====
const INPUT_MODEL_VERSION_REUSE_TOTALS = 1; // section_reuse_totals
const INPUT_MODEL_VERSION_BASE = 1;        // section_metrics v1 provides I_base + genre vectors
const OUTPUT_MODEL_VERSION = 2;            // section_metrics v2 output

const W_NT = 0.65;
const W_OT = 0.35;

const M_MIN = 1.0;
const M_MAX = 1.60; // locked
...
async function fetchP95Reuse(column) {
  // p95 over reuse totals rows only (nonzero sections); missing rows => zero handled later
  ...
  while (true) {
    ...
    for (const r of data) {
      map.set(r.section_id, {
        ...
        space: Number(r.space_score),
        structural: Number(r.structural_score),
        gNarr: Number(r.genre_narrative),
        gInstr: Number(r.genre_instruction),
        gArg: Number(r.genre_argument),
        gPoet: Number(r.genre_poetry),
        gList: Number(r.genre_list),
        assess: Number(r.assessability_score),
        baseImportance: Number(r.importance_score)
      });
    }
...
async function main() {
  console.log(`Compute v2 importance (section_metrics model_version=${OUTPUT_MODEL_VERSION})`);

  const booksMap = await fetchBooksMap();
  const reuseMap = await fetchReuseMap();
  const v1Map = await fetchV1Map();

  console.log(`Books: ${booksMap.size}`);
  console.log(`Reuse rows: ${reuseMap.size} (missing => zero reuse)`);
  console.log(`V1 rows: ${v1Map.size}`);

  if (v1Map.size < 1000) {
    throw new Error('V1 metrics look incomplete. Aborting to avoid constraint violations.');
  }
  ...
  console.log('Cleared existing v2 rows.');
  ...
  while (true) {
    ...
    for (const s of secs) {
      ...
      buffer.push({
        ...
        // carry forward v1 components (preserves constraints)
