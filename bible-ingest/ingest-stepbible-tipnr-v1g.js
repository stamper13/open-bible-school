...
function parseRefToken(token) {
  ...
  const book_code = m[1].toUpperCase();
...
async function loadValidBookCodes() {
  const { data, error } = await supabase
    .from('scripture_books')
    .select('book_code');

  if (error) throw error;
  return new Set((data || []).map(r => (r.book_code || '').toUpperCase()));
}

async function upsertEntity({ entity_type, unique_name, display_name, tipnr_key, header_fields }) {
  // Upsert then fallback select if data is null
  const { data, error } = await supabase
    .from('bible_entities')
    .upsert(
  ...
  if (!fb.data?.id) throw new Error(`Failed to fetch entity id after upsert for unique_name=${unique_name}`);
  return fb.data.id;
}

async function upsertFormAndGetId({ entity_id, unique_tag, hebrew_greek, esv_name, step_link, refs_raw }) {
  const payload = {
    entity_id,
    unique_tag,
  ...
  const { data, error } = await supabase
    ...
    .upsert(payload, { onConflict: 'entity_id,unique_tag' })
    .select('id')
    .maybeSingle();
  ...
  // Fallback fetch (this is the key fix for your v1f crash)
  const fb = await supabase
    .from('bible_entity_forms')
    .select('id')
    .eq('entity_id', entity_id)
    .eq('unique_tag', unique_tag)
    .maybeSingle();

  if (fb.error) throw fb.error;
  if (!fb.data?.id) {
    throw new Error(`Failed to fetch form id after upsert for entity_id=${entity_id}, unique_tag=${unique_tag}`);
  }
  return fb.data.id;
}

function dedupeRefs(rows) {
  // Deduplicate by (form_id, ref)
  const map = new Map();
  for (const r of rows) {
...
async function main() {
  console.log('TIPNR ingest v1g starting…');
  console.log('Reading:', TIPNR_PATH);

  const validBooks = await loadValidBookCodes();
  console.log(`Valid book codes loaded: ${validBooks.size}`);
  ...
  let skippedBadRefs = 0;
  let skippedUnknownBooks = 0;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line0.startsWith('\t')) {
      ...
      if (currentType === 'place') {
        ...
        refs_raw = c4 || null;
        step_link = c5 || null;
        if (isUrl(refs_raw) && !isUrl(step_link)) {
          const tmp = refs_raw;
          refs_raw = step_link;
          step_link = tmp;
        }
      ...
      const form_id = await upsertFormAndGetId({
      ...
      if (refs_raw) {
        ...
        for (const tok of tokens) {
          ...
          if (parsed.chapter < 1 || parsed.verse < 1) {
            skippedBadRefs++;
            continue;
          }

          if (!validBooks.has(parsed.book_code)) {
            skippedUnknownBooks++;
            continue;
          }
  ...
  console.log('Skipped bad refs (chapter/verse < 1):', skippedBadRefs);
  console.log('Skipped unknown book codes:', skippedUnknownBooks);
