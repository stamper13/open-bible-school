...
const INPUT_MODEL_VERSION_BASE = 1;
const OUTPUT_MODEL_VERSION = 2;
...
const PAGE_SIZE = 1000;     // <= IMPORTANT (avoid server-side caps)
...
async function fetchBooksMap() {
  ...
  while (true) {
    ...
    // SAFE pagination: advance by returned length
    offset += data.length;
...
async function main() {
  ...
  while (true) {
    ...
    offset += secs.length;              // SAFE pagination
    process.stdout.write(`Scanned sections: ${offset}\r`);
    if (secs.length < PAGE_SIZE) break; // last page
  }
