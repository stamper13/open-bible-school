...
// Looks like: Aaron@Exo.4.14=H0175A  OR  Abraham@Gen.11.26  (sometimes no =key)
...
async function upsertRefsBulk(rows) {
  ...
  const { error } = await supabase
    ...
    .upsert(rows, { onConflict: 'form_id,ref' });
...
async function main() {
  console.log('TIPNR ingest v1c starting…');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Marker lines like "$========== PERSON(s)" (may have leading spaces)
    ...
    if (line0.startsWith('\t')) {
      ...
      // Subrecord formats differ slightly by type; we normalize by heuristics
    ...
    // Example: Aaron@Exo.4.14=H0175A\tMale living at...
    if (currentType && ltrim.includes('\t')) {
      const cols = splitTabs(ltrim);
      const unique_name = (cols[0] || '').trim();

      // Skip field-description header rows like "UniqueName\tDescription..."
