export const ASSESS_RESPONSIVE_STYLES = `        /* ============================================================
        ...
        @media (max-width: 640px) {
          ...
          /* auto, never hidden. Pinning assumes the content fits, and when
             that assumption is wrong — a long prompt, a browser whose chrome
             is taller than svh accounts for, an accessibility text size —
             hidden makes whatever overflowed unreachable, including the
             answer button. Reported on an iPhone where the card ran under
             Safari's toolbar and no finger drag could recover it. With auto
             the pin still holds every case where the content does fit, and
             degrades to an ordinary scroll where it does not. */
          /* No viewport pin here any more, deliberately. Capping the page at
             100svh with the scene absorbing the slack made the answers land
             in the same spot every question, but it assumed the content
             always fits. On a real iPhone it did not: the answer button came
             to rest under Safari's toolbar and no gesture could reach it,
             because the pin also removed the page scroll that would have
             rescued it. The tightening below is what buys the room now, and
             the page keeps its ordinary scroll as the backstop for whatever
             the tightening cannot cover — long prompts, big accessibility
             text, browser chrome taller than svh predicts. */
          ...
          /* The card becomes a column that does not outgrow the scene, so in
             the common case the page has nothing to scroll. Only the prompt
             is allowed to flex:
