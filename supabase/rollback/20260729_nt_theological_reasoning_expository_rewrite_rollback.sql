-- Roll back the NT Theological Reasoning expository rewrite.
-- Refuse once a replacement question has answer history.
...
begin
  ...
  into backup_count, backup_row_count
  ...
  if backup_count <> 1
     or backup_row_count <> 21
     or replacement_answers <> 0
  then
    raise exception using
      errcode = 'P0001',
      message = format(
        'NT theology rollback refused: backups=%s/1 backup_rows=%s/21 replacement_answers=%s/0.',
        backup_count,
        backup_row_count,
        replacement_answers
      );
