-- Roll back the NT Promise & Prophecy expository rewrite.
...
begin
  ...
  if backup_count <> 1
     or backup_row_count <> 25
     or replacement_answers <> 0
  then
    raise exception using
      errcode = 'P0001',
      message = format(
        'NT Promise rollback refused: backups=%s/1 backup_rows=%s/25 replacement_answers=%s/0.',
        backup_count,
        backup_row_count,
        replacement_answers
      );
