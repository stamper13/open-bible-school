-- Router v6, step 8: dimension-precedence follow-up for the v6 ranker.
--
-- Steps 1-5 were applied to at least one Supabase branch before the
-- cold-start/router-precedence diagnosis was corrected. Supabase will not
-- rerun an already-applied migration version, so this follow-up replaces the
-- v6 ranker with the corrected body even when 20260822140400 is already in
-- schema_migrations.
--
...
begin
  ...
  then
    raise exception using
      errcode = 'P0001',
      message = 'Router v6 step 8 prerequisites are missing; nothing was changed.';
  end if;
end
$$;
