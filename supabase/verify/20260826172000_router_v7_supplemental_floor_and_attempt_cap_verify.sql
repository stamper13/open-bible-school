...
begin
  ...
  with checks(name, ok) as (
    values
      (
        'V7 supplemental candidate source is installed',
        coalesce(v_function, '') like '%V7_LOW_EVIDENCE_SUPPLEMENTAL%'
      ),
      (
        'supplemental source excludes exact history',
        coalesce(v_function, '') like '%where prior.generated_question_id = question.generated_question_id%'
      ),
      (
        'supplemental source excludes similarity history',
        coalesce(v_function, '') like '%prior.similarity_key =%'
      ),
      (
        'post-200 attempt section cap is installed',
        coalesce(v_function, '') like '%post-200 attempt section cap%'
      ),
      (
        'late long-run section brake was strengthened',
        coalesce(v_function, '') like '%then 5%'
      ),
      (
  ...
  if jsonb_array_length(v_failures) > 0 then
    raise exception 'FAIL: V7 supplemental floor/cap verification failed: %', v_failures;
  end if;
end;
$$;
