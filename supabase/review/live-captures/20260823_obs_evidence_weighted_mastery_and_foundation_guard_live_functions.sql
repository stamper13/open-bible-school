-- Captured from live Supabase project idyavsqksxtgogpfwlei on 2026-08-23 after Claude-applied migration.
-- Function: obs_focused_mastery_raw(numeric,numeric,numeric,boolean,boolean,boolean)
CREATE OR REPLACE FUNCTION public.obs_focused_mastery_raw(p_stage_1_accuracy numeric, p_stage_2_accuracy numeric, p_stage_3_accuracy numeric, p_stage_1_available boolean, p_stage_2_available boolean, p_stage_3_available boolean)
 RETURNS numeric
 LANGUAGE sql
...
-- Function: obs_get_current_focus_path(uuid)
CREATE OR REPLACE FUNCTION public.obs_get_current_focus_path(p_user_id uuid)
 RETURNS TABLE(level text, depth integer, node_id uuid, node_key text, label text, book_code text, rank integer, is_focus boolean, answered integer, display_score integer, state text, reference text, start_ch integer, start_vs integer, end_ch integer, end_vs integer, focus_mode text, book_probes_answered integer, book_probes_correct integer)
 LANGUAGE sql
...
-- Function: obs_get_ladder_state_v1(uuid)
CREATE OR REPLACE FUNCTION public.obs_get_ladder_state_v1(p_user_id uuid)
 RETURNS TABLE(unit_key text, sequence_order integer, section_key text, section_name text, book_code text, book_name text, label text, start_chapter integer, end_chapter integer, is_foundation boolean, answered integer, display_score integer, required_answers integer, required_score integer, state text, is_focus boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- Function: obs_get_user_recommendation_v2(uuid)
CREATE OR REPLACE FUNCTION public.obs_get_user_recommendation_v2(p_user_id uuid)
 RETURNS TABLE(unit_key text, label text, section text, book_code text, start_chapter integer, end_chapter integer, sequence_order integer, is_foundation boolean, answered integer, correct integer, raw_score numeric, display_score integer, baseline_display_score_required integer, retest_question_target integer, focus_text text, reason text, recommendation_kind text, dimension_key text, dimension_label text, dimension_short_label text, dimension_answered integer, dimension_correct integer, dimension_display_score integer, dimension_available_questions integer, dimension_focus_text text)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- Function: obs_unit_has_foundation_items(text)
