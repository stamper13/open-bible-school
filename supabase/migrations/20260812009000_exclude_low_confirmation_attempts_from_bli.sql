-- If a completed assessment has only a small fraction of client-confirmed
-- scoring rows, do not let the few confirmed rows stand alone as a BLI score.
-- This handles legacy attempts submitted through the old MCQ path where most
-- answers were intentionally marked server_unconfirmed_v1.

with attempt_coverage as (
  select
    attempt.id as attempt_id,
    attempt.user_id,
    coalesce(attempt.answered_count, 0) as answered_count,
    count(answer.id) filter (where answer.scoring_eligible)::integer
      as eligible_count
  from public.assessment_attempts attempt
  left join public.assessment_answers answer
    on answer.attempt_id = attempt.id
   and answer.user_id = attempt.user_id
  where coalesce(attempt.is_complete, false)
    and coalesce(attempt.answered_count, 0) >= 10
  group by attempt.id, attempt.user_id, attempt.answered_count
),
low_confirmation_attempts as (
  select attempt_id, user_id
  from attempt_coverage
  where eligible_count < answered_count * 0.75
)
update public.assessment_answers answer
set scoring_eligible = false,
    scoring_exclusion_reason = 'attempt_incomplete_client_confirmation_coverage',
    scoring_eligibility_reviewed_at = now()
from low_confirmation_attempts bad_attempt
where answer.attempt_id = bad_attempt.attempt_id
  and answer.user_id = bad_attempt.user_id
...
as $$
  ...
  attempt_quality as (
    select
      attempt.id as attempt_id,
      coalesce(attempt.is_complete, false) as is_complete,
      coalesce(attempt.answered_count, 0) as answered_count,
      count(answer.id) filter (where answer.scoring_eligible)::integer
        as eligible_count
    from public.assessment_attempts attempt
    ...
    group by attempt.id, attempt.is_complete, attempt.answered_count
  ),
  rows as (
    select
      evidence.*,
    ...
    join attempt_quality quality
      on quality.attempt_id = evidence.attempt_id
    where evidence.user_id = p_user_id
      ...
      and (
        not quality.is_complete
        or quality.answered_count < 10
        or quality.eligible_count >= quality.answered_count * 0.75
      )
...
comment on function public.obs_compute_scoped_bli(uuid, text, timestamptz) is
  'Computes cumulative scoped BLI from scoring-eligible answers, excluding completed attempts with low client-confirmed coverage.';
