...
declare
  ...
  v_selected_review_text text;
  v_correct_review_text text := 'Each displayed book belongs in its Hebrew Bible section: Torah, Former Prophets, Latter Prophets, or Writings.';
begin
  ...
  v_review_prompt := case
    when upper(coalesce(v_attempt.testament, 'OT')) = 'NT'
      then 'Drag each New Testament book to its correct division.'
    else 'Using Hebrew Bible divisions, drag each book to its correct section.'
  end;

  if upper(coalesce(v_attempt.testament, 'OT')) = 'NT' then
    v_correct_review_text :=
      'Each displayed book belongs in its New Testament division: Gospels & Acts, Pauline Epistles, General Epistles, or Apocalypse.';
  end if;
  ...
  v_selected_review_text := public.obs_section_sort_review_text(p_assignments);
  ...
  insert into public.assessment_answers (
    ...
    now(),
    p_assignments,
    v_selected_review_text,
    'A',
    v_correct_review_text,
    v_review_prompt,
    'section_sort_drag_drop_v2'
  )
  on conflict (attempt_id, question_id) do update
  set delivered_choices_snapshot = excluded.delivered_choices_snapshot,
      selected_choice_text_snapshot = excluded.selected_choice_text_snapshot,
      correct_choice_id_snapshot = excluded.correct_choice_id_snapshot,
      ...
      delivery_contract = excluded.delivery_contract
...
comment on function public.obs_submit_section_sort_answers(uuid, uuid, jsonb) is
  'Submits section-sort drag/drop answers and stores review snapshots matching the displayed drag/drop interaction without double-scoring retries.';
