...
begin
  ...
  loop
    ...
    if v_correct is null then
      ...
      v_correct := case
        when v_book.testament = 'NT' and v_book.nt_division = 'Gospels_Acts' then 'GOSPELS_ACTS'
        when v_book.testament = 'NT' and v_book.nt_division = 'Pauline' then 'PAULINE'
        when v_book.testament = 'NT' and v_book.nt_division = 'General' then 'GENERAL'
        when v_book.testament = 'NT' and v_book.nt_division = 'Apocalypse' then 'APOCALYPSE'
        when v_book.book_code in ('GEN','EXO','LEV','NUM','DEU') then 'TORAH'
        when v_book.book_code in ('JOS','JDG','RUT','1SA','2SA','1KI','2KI') then 'FORMER'
        when v_book.book_code in ('ISA','JER','LAM','EZK','DAN','HOS','JOL','AMO','OBA','JON','MIC','NAM','HAB','ZEP','HAG','ZEC','MAL') then 'LATTER'
        when v_book.testament = 'OT' then 'WRITINGS'
        else null
      end;
    end if;
...
comment on function public.obs_submit_section_sort_answers(
  ...
  'Submits section-sort drag/drop answers for both metadata-backed screens and older MCQ-style book categorization prompts.';
