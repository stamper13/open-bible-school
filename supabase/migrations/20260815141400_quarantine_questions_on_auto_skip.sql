-- Quarantine a question as soon as the emergency auto-skip path is used.
--
-- Broken questions should not count toward the learner's target, but they also
-- must not remain eligible and trap the learner in a stream of malformed items.
...
begin
  ...
  set question_type = case
      ...
      payload = jsonb_set(
        ...
        '{quarantine_reason}',
        to_jsonb(concat(
          'Auto-quarantined by obs_skip_broken_assessment_question after submit failure: ',
          coalesce(p_error_code, 'unknown'),
          ' ',
          left(coalesce(p_error_message, 'unknown'), 240)
        ))
      )
  where question.id = p_generated_question_id;
...
comment on function public.obs_skip_broken_assessment_question(
  ...
  'Logs a broken assessment question, auto-quarantines it, marks it skipped without scoring using schema-valid choice ids, and keeps the attempt target stable.';
