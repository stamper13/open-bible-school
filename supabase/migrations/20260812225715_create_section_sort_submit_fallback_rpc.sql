...
begin
  ...
  if not found then
    raise exception using errcode = '22023', message = 'Section-sort screen is not configured';
  end if;
...
comment on function public.obs_submit_section_sort_answers(
  ...
  'Fallback RPC for section-sort drag/drop screens; records one all-or-nothing screen answer when per-label child scoring is not deployed.';
