...
begin
  ...
  insert into public.question_reports (
    ...
    concat_ws(E'\n',
      ...
      nullif(concat('Error code: ', coalesce(p_error_code, 'unknown')), 'Error code: '),
      nullif(concat('Error message: ', coalesce(p_error_message, 'unknown')), 'Error message: '),
  ...
  set scoring_eligible = false,
      ...
      selected_choice_id = '__BROKEN_QUESTION__',
      is_correct = false,
      is_idk = true,
  ...
  into v_eligible_answered, v_correct, v_broken_count
  from public.assessment_answers answer
  where answer.attempt_id = p_attempt_id
    and answer.user_id = v_user_id
    and (
      answer.scoring_eligible
      or answer.scoring_exclusion_reason = 'auto_skipped_broken_question'
    );
  ...
  set answered_count = v_eligible_answered,
      ...
      target_question_count = greatest(
        coalesce(attempt.target_question_count, v_base_target),
        v_base_target + v_broken_count
      ),
      total_count = greatest(
        coalesce(attempt.total_count, v_base_target),
        v_base_target + v_broken_count
      ),
      is_complete = false,
      completed_at = null
  ...
  return query select
    v_eligible_answered,
    v_correct,
    greatest(coalesce(v_attempt.target_question_count, v_base_target), v_base_target + v_broken_count);
end;
$function$;
...
comment on function public.obs_skip_broken_assessment_question(
  ...
  'Logs a broken assessment question, marks it skipped without scoring, and extends the attempt target so learners still receive the intended number of scored questions.';
