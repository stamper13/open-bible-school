-- Router v6, step 5 of 8: the mode-aware candidate ranker.
--
-- v6 wraps v5 exactly as v5 wraps v4. It adds no new selector and rewrites
-- nothing while it is unreachable. Once active, cold_start keeps the fast
-- section scan for only the opening items; after that v6 reranks a wider v5
-- pool so dimension debt can shape the distribution instead of being trapped
-- behind v5's hard route-priority buckets.
--
...
begin
  ...
  then
    raise exception using
      errcode = 'P0001',
      message = 'Router v6 step 5 prerequisites are missing; nothing was changed.';
  end if;
end
$$;
...
comment on function public.obs_rank_ot_assessment_candidates_v6(uuid, uuid, text, integer, timestamptz, integer) is
  'Mode-aware wrapper over v5. In cold_start/sweep it widens the v5 pool and '
  'lets dimension debt outrank v5 route-bucket precedence for ordinary ranked '
  'items; in campaign mode it promotes candidates that serve the open campaign '
  'phase and sit inside its stage window, subject to a per-attempt cap. STABLE: '
  'writes nothing.';
